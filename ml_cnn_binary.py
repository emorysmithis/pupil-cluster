...
class CNN1DHybrid(nn.Module):
    def __init__(self, seq_channels, hand_feat_dim, hidden_dim=128, dropout=0.3):
        super().__init__()

        # 1D CNN branch for time series
        self.cnn = nn.Sequential(
            nn.Conv1d(seq_channels, 64, kernel_size=21, padding=10),
            nn.ReLU(),
            nn.MaxPool1d(2),
        ...
        self.final = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, 1)  # Binary classification (BCEWithLogitsLoss)
        )

    def forward(self, X_seq, X_feat):
        # X_seq: [batch, seq_len, channels] -> [batch, channels, seq_len]
        X_seq = X_seq.permute(0, 2, 1)
        cnn_out = self.cnn(X_seq)
        
        # Adaptive global average pooling to get fixed size [batch, channels]
        cnn_out = F.adaptive_avg_pool1d(cnn_out, 1).squeeze(2)
...
def evaluate_accuracy(model, loader, device="cpu"):
    ...
    with torch.no_grad():
        for X_seq_batch, X_feat_batch, y_batch in loader:
...
def verify_alignment(X_seq, X_feat, y, features_df_filtered, pupil_cols, mask_cols, trial_df_dict_filtered):
    all_cols = pupil_cols + mask_cols
    n_trials = len(features_df_filtered)
    assert X_seq.shape[0] == n_trials
    assert X_feat.shape[0] == n_trials
    assert len(y) == n_trials

    for i in range(n_trials):
        # Check y
        y_expected = features_df_filtered.iloc[i]["NumCorrect"]
        y_actual = y[i] if isinstance(y[i], int) else y[i].item()
        assert y_actual in [0,1], f"Unexpected y value {y_actual} at index {i}"

        # Check hand features
        hand_cols = [c for c in features_df_filtered.columns if c not in ["subject_session_trial","NumCorrect"]]
        feat_expected = pd.to_numeric(
        features_df_filtered.iloc[i][hand_cols], errors='coerce'
        ).values.astype(float)

        feat_actual = X_feat[i].cpu().numpy().astype(float) \
            if isinstance(X_feat[i], torch.Tensor) else X_feat[i].astype(float)

        # Now allclose works
        assert np.allclose(feat_actual, feat_expected, atol=1e-5), \
            f"X_feat mismatch at index {i}"


        # Optional: check X_seq channels
        assert X_seq[i].shape[1] == len(all_cols), f"X_seq channel mismatch at index {i}"

    for idx, trial_id in enumerate(features_df_filtered["subject_session_trial"]):
        df = trial_df_dict_filtered[trial_id]

        # first time step, first pupil channel (LeftEye)
        seq_left_first = X_seq[idx, 0, 0].item()  
        df_left_first  = df["SmoothedDilationPupilLeftEye"].values[0]

        assert abs(seq_left_first - df_left_first) < 1e-5, \
            f"Mismatch at index {idx}, trial_id {trial_id}"



    print(f"✅ Alignment verified for {n_trials} trials.")
...
if __name__ == "__main__":
    ...
    # Step 0: Convert to binary labels
    labels_binary = np.where(y_all <= 2, 0,
                    np.where(y_all >= 4, 1, -1))  # -1 marks ambiguous trials

    # Step 1: Identify valid rows
    valid_idx = labels_binary != -1

    # Step 2: Filter *everything* with the same mask
    X_seq = X_seq[valid_idx]                  # shape [n_valid_trials, seq_len, channels]
    X_feat_all = X_feat_all[valid_idx]        # features
    y_all_binary = labels_binary[valid_idx]   # filtered labels
    

    # Replace y_all with the binary one
    y_all = y_all_binary

    features_df_filtered = features_df.iloc[valid_idx].reset_index(drop=True)
    kept_ids = set(features_df_filtered["subject_session_trial"])
    trial_df_dict_filtered = {k: v for k, v in trial_df_dict.items() if k in kept_ids}


    print(X_seq.shape)
    print(X_feat_all.shape)
    print(len(y_all))
    verify_alignment(X_seq, X_feat_all, y_all, features_df_filtered, pupil_cols, mask_cols, trial_df_dict_filtered)
    ...
    # ----------------------------
    # Normalize hand-engineered features (train stats)
    # ----------------------------
    ...
    # ----------------------------
    # Normalize pupil channels (train stats only)
    # ----------------------------
    ...
    # verify seq, feat, label alignment - DONE 
    # TODO: check blink percentage 
    # TODO: check blink interpolation? 
    # TODO: try velocity as feature? I think this might be in there. 
    # check to see if multi-class -> binary is better - DONE, nope, not better 
    # TODO: check to see if features are helping, which features are helping 
    # TODO: check to see if all of time series helping 
