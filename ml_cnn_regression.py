...
if __name__ == "__main__":
    ...
    # Initialize lists to store all predictions (continuous values) and ground truth (continuous values)
    all_predictions = []
    all_ground_truths = []
    ...
    with torch.no_grad():
        for X_seq_batch, X_feat_batch, y_batch in test_loader:
            ...
            # Store the continuous values
            # .detach().cpu().numpy() converts the tensor to a standard NumPy array for storage
            all_predictions.append(preds.detach().cpu().numpy())
            all_ground_truths.append(y_batch.detach().cpu().numpy())

    test_loss /= len(test_loader.dataset)

    # --- Post-Processing ---

    # Concatenate all batch results into single NumPy arrays, and flatten them
    final_predictions = np.concatenate(all_predictions, axis=0).flatten()
    final_ground_truths = np.concatenate(all_ground_truths, axis=0).flatten()

    # Recalculate Error using the flattened arrays
    error = final_predictions - final_ground_truths

    print("--- Sample Predicted vs. Actual Values (First 10) ---")
    for i in range(10): 
        # Remove all [0] indices
        print(f"Sample {i+1}: Predicted = {final_predictions[i]:.4f}, "
            f"Actual = {final_ground_truths[i]:.4f}, "
            f"Error = {error[i]:.4f}")

    # Optional: Create a DataFrame for easy inspection
    results_df = pd.DataFrame({
        'Predicted': final_predictions.flatten(), 
        'Actual': final_ground_truths.flatten(),
        'Error': (final_predictions - final_ground_truths).flatten()
    })
